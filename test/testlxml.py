...
proxylist = []
root = etree.HTML(html)
proxys = root.xpath(config.parserList[1]['pattern'])
for proxy in proxys:
        ip = proxy.xpath(config.parserList[1]['position']['ip'])[0].text
        port = proxy.xpath(config.parserList[1]['position']['port'])[0].text
        print(ip+port)